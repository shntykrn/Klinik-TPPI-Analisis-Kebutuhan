...
const BuatSuratSakit = () => {
  ...
  const saatInputBerubah = (e) => {
    ...
    aturDataFormulir((sebelumnya) => ({ ...sebelumnya, [name]: value }));
  };

  const simpanSurat = async () => {
    try {
      const formData = new FormData();
      for (const key in dataFormulir) {
        formData.append(key, dataFormulir[key]);
      }

      // Jika belum pilih file, pakai dummy PDF lokal dari public folder
      if (fileSurat) {
        formData.append('file', fileSurat);
      } else {
        alert('Mohon unggah file surat (PDF/Gambar).');
        return false;
      }
      ...
      if (!respons.ok) throw new Error('Gagal menyimpan surat');
